/**
 * POST /api/generate-toolbox-talk — the Toolbox Talk Generator, public and
 * Academy-member modes.
 *
 * Node runtime (not edge): PDF rendering needs headless Chromium.
 *
 * Public (no accessToken): gated only by email — rate-limited per email + IP
 * to protect API cost. Captures the lead (source-tagged) into the same table
 * used for rate limiting.
 *
 * Academy member (accessToken from the Kajabi welcome email, see
...
export default async function handler(req: VercelLikeRequest, res: VercelLikeResponse): Promise<void> {
  ...
  const accessToken = (body.accessToken ?? "").toString().trim().slice(0, 128) || undefined;
  ...
  let email: string;
  let isMember = false;
  let memberTier: string | undefined;
  ...
  try {
    const rateLimit = isMember
      ? await checkMemberRateLimit(email, ip)
    ...
    recordGeneration({
      ...
      site: site || undefined,
      title: talk.title,
      source: isMember ? "academy-member" : "toolbox-talk-generator",
      utm: isMember ? { tier: memberTier ?? "unknown" } : utm,
    }).catch(() => {
