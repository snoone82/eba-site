/**
 * EBA Toolbox Talk Generator.
 *
 * Two modes, one page:
 *  - Public (no ?access= param): email-capture, rate-limited, matches the free
 *    tier promised on /ai-tools.
 *  - Academy member (?access=<token> from the Kajabi welcome email): no email
 *    step, effectively unlimited, matches "the full version included with
 *    every Academy enrolment" promised on the FAQ/pricing pages.
 */
...
import {
  NAVY, NAVY_RGB, CREAM, WHITE, COBALT, COBALT_RGB, COBALT_ON_DARK,
  CORAL, RUST, COLORS,
...
import { Sparkles, Check, Download, AlertCircle, Crown } from "lucide-react";
...
/** ?access=<token> from the Kajabi welcome email — undefined for public visitors. */
...
function GeneratorForm({
  ...
  accessToken?: string;
  onResult: (r: GenerateResult, input: { title?: string }) => void;
}) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!topic.trim() || (!isMember && !email.trim())) return;
    setFormError("");
    setLoading(true);
    track(isMember ? "toolbox_talk_member_generate_submit" : "toolbox_talk_generate_submit");
    ...
    try {
      const res = await fetch("/api/generate-toolbox-talk", {
        ...
        body: JSON.stringify(
          isMember
            ? { topic, trade, site, accessToken }
            : { topic, trade, site, email, utm: getStoredUtm() },
        ),
      });
      ...
      if (res.status === 501) {
        ...
        if (body.invalidTopic) {
          ...
          track(isMember ? "toolbox_talk_member_generated" : "toolbox_talk_generated");
    ...
    <form onSubmit={onSubmit} style={{ display: "grid", gap: "16px" }}>
      {isMember && (
        <div style={{
          ...
          background: `rgba(${COBALT_RGB},0.08)`, border: `1px solid rgba(${COBALT_RGB},0.25)`,
          borderRadius: "10px", padding: "12px 16px",
        }}>
          <Crown size={16} color={COBALT} style={{ flexShrink: 0 }} />
          <p style={{ fontFamily: "'Poppins', sans-serif", fontSize: "13px", color: NAVY, margin: 0, fontWeight: 600 }}>
            Academy member — unlimited generations, no email needed.
          </p>
        </div>
      )}
      <div>
      ...
      {formError && <p style={{ color: RUST, fontSize: "13px", margin: 0 }} role="alert">{formError}</p>}
      <button
...
function ResultPanel({ result, requestedTitle }: { result: GenerateResult; requestedTitle?: string }) {
  ...
  if (result.kind === "rate-limited") {
    return (
      <div style={{ ...panelBase, background: "rgba(127,97,27,0.06)", border: `1px solid rgba(127,97,27,0.25)` }}>
        <div style={{ display: "flex", gap: "10px", alignItems: "flex-start" }}>
          <AlertCircle size={18} color={RUST} style={{ flexShrink: 0, marginTop: "2px" }} />
      ...
      <div style={{ ...panelBase, background: "rgba(127,97,27,0.06)", border: `1px solid rgba(127,97,27,0.25)` }}>
        <div style={{ display: "flex", gap: "10px", alignItems: "flex-start" }}>
          ...
          <p style={{ fontSize: "14px", color: NAVY, margin: 0 }}>
            This access link isn't valid — it may have expired or been replaced by a newer one
            (each purchase issues a fresh link). Use the free version on this page instead, or{" "}
            <Link href="/contact" style={{ color: COBALT }}>contact us</Link> if you think this is a mistake.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ ...panelBase, background: WHITE, border: `1px solid rgba(${NAVY_RGB},0.1)`, boxShadow: "0 30px 60px -32px rgba(0,0,0,0.28)" }}>
      ...
      <div style={{ display: "inline-block", border: `1px solid ${RUST}`, color: RUST, fontSize: "11px", fontWeight: 600, padding: "3px 10px", borderRadius: "999px", marginBottom: "14px" }}>
        {talk.compliance}
      </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", marginTop: "16px", alignItems: "center" }}>
        ...
        <span style={{ display: "inline-flex", alignItems: "center", gap: "6px", fontSize: "13px", color: `rgba(${NAVY_RGB},0.65)` }}>
          <Check size={15} color={emailed ? COLORS.mint : `rgba(${NAVY_RGB},0.4)`} />
    ...
    <div style={{ fontFamily: "'Poppins', sans-serif", background: CREAM, color: NAVY, overflowX: "hidden" }}>
      ...
      <section style={{ paddingTop: isMobile ? "104px" : "150px", paddingBottom: isMobile ? "56px" : "88px", background: DARK_GRADIENT, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: "1200px", margin: "0 auto", padding: isMobile ? "0 20px" : "0 40px" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", fontWeight: 700, fontSize: "11px", letterSpacing: "0.16em", textTransform: "uppercase", color: COBALT_ON_DARK, marginBottom: "16px" }}>
            {isMember ? <Crown size={15} strokeWidth={2} /> : <Sparkles size={15} strokeWidth={2} />}
            {isMember ? "Academy member · Full version" : "Free tool · No purchase"}
          </div>
          <h1 style={{ fontFamily: "var(--eba-heading)", fontWeight: 900, fontSize: isMobile ? "2.1rem" : "clamp(2.4rem, 4.6vw, 3.6rem)", lineHeight: 1.06, letterSpacing: "-0.02em", color: ON_DARK, margin: "0 0 16px", maxWidth: "760px" }}>
            {isMember ? "Your Toolbox Talk Generator." : "The free Toolbox Talk Generator."}
          </h1>
          <p style={{ fontSize: isMobile ? "15px" : "17px", lineHeight: 1.65, color: `rgba(${ON_DARK_RGB},0.75)`, maxWidth: "560px", margin: 0 }}>
            {isMember
              ? "Unlimited, EBA-branded UK toolbox talks with a sign-off sheet — included with your Academy enrolment. No email needed, generate as many as you like."
              : "Type any site hazard or task. Get a genuinely useful, EBA-branded UK toolbox talk with a sign-off sheet in about a minute — on screen instantly, and a copy in your inbox."}
          </p>
        </div>
      </section>
        ...
        <section style={{ background: NAVY, padding: isMobile ? "48px 20px" : "64px 40px" }}>
          <div style={{ maxWidth: "900px", margin: "0 auto", textAlign: isMobile ? "left" : "center" }}>
            ...
            <Link
              ...
              style={{ display: "inline-block", background: CORAL, color: "#0A0A0A", textDecoration: "none", fontWeight: 700, fontSize: "15px", padding: "14px 30px", borderRadius: "10px" }}
            >
